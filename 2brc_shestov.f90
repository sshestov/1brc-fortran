!! in this version we are trying to use a SWAR-based technique to find a semicolon and replace FINDLOC
!! see https://fortran-lang.discourse.group/t/1-billion-row-challenge-in-fortran-mmap-openmp-branchless-scan/7254/19
...
integer                            :: tempi
integer(kind=1)                    :: zcode, scode, nlcode, dcode, mcode           ! 8bit int of symbols
...
integer(kind=8)                    :: r                                            ! 8-byte part of i11 array

print'(A, /, 17(" "), I4, I6, I8, I8, I6, I7)', "Storage sizes for char, int(1), int(2), int, int(4), real [bits]: ", &
        storage_size(nl_t), storage_size(10_1), storage_size(10_2), storage_size(tempi), storage_size(10_4), storage_size(temp)
...
fd=10
inquire( file='measurements.txt', iostat=iostat, size=len)
print*, "File 'measurements.txt' has size ", len, " bytes"

!len = 2039 !2040
off = 0

print*, "******************* Using C functions to open and mmap the file *******************"
cptr = invoke_mmap(0,len,PROT_READ,MAP_PRIVATE,fd,off)
!fd = open(c_loc(filename),O_RDONLY)
...
call c_f_pointer(cptr,x11,(/len/))
if (.not. associated(x11)) print*, "x11 IS NOT associated with anything :("
call c_f_pointer(cptr,i11,[len])
if (.not. associated(i11)) print*, "i11 IS NOT associated with anything :("

write(*,'(A,I3,A)') "*************** Splitting the file in ", Nchunks, " chunks. Each OMP thread should receive its chunk ***************"
chunk_size=len / Nchunks  ;  cpos=1
do i=1,Nchunks-1                                                                         
...
ch_start(1)=1
ch_end(Nchunks)=len
ch_end(Nchunks)=findloc(i11(len-100:),scode,DIM=1)-1 + len-100
...
write(*,*) "******************************************* Running ******************************************* "
!$OMP PARALLEL DO REDUCTION(min:temp_min_i) REDUCTION(max:temp_max_i) REDUCTION(+:temp_cum_i,stations_cnt) PRIVATE(cpos,pos_s,pos_s1,pos_nl,str,i,j,r,id,temp,tempi,d2p,d3p,d4p,m1p,name_i16) DEFAULT(shared) SCHEDULE(DYNAMIC)
do chunk_num=1,Nchunks
  cpos = ch_start(chunk_num)
  !! now the logic is different: since anyway we need to search through i11, we just do it in 8-byte inc, and change cpos when a semicolon is found
  do j=ch_start(chunk_num), ch_end(chunk_num), 8   
    ...
    pos_s1 = t_index(r)                           !! Initially this is a position inside 8-byte r
    if (pos_s1 > 0) then
      pos_s1 = j+pos_s1-1 - cpos                  !! but we need position with respect to cpos
      !write(*,'(A, I4, A)',advance='NO') "J = ", j, ":  "
      !write(*,'(A, I4)',advance='NO') "Position of ';' (OLD:NEW): ", pos_s   ;   write(*,'(I4)') pos_s1
      ...
      pos_nl = d2p*(pos_s+4) + d3p*(pos_s+5) + d4p*(pos_s+6) + 1                        !! use old definition 1..??;  positions of the semicolon and the NEW LINE are counted from the cpos
      ...
      !write(*,'(" : ",*(A1))',advance='NO') x11(cpos+pos_s+1:cpos+pos_nl-2), "|"        !! exact position of the temperature is cpos+pos_s+1:cpos+pos_nl-2
      ...
      cpos = cpos+pos_nl
      !if (cpos >= ch_end(chunk_num)) exit    !! we do not this anymore since j scrolls through the whole i11 chunk
    end if
  end do
end do
!$OMP END PARALLEL DO

tempi=munmap(cptr, len)
...
integer function t_index(x)
!integer function t_index(x, y)
    !integer(8), intent(in) :: x, y
    integer(8), intent(in) :: x
    integer(8), parameter :: sc = int(z'3b3b3b3b3b3b3b3b',8)   ! sc=int(z'3b...',8) !! hex 3b == 59 = iachar(';')
